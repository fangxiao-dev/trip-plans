...
def get_db_dir() -> Path:
    """Return a project-local Codex data directory unless explicitly overridden."""
    configured_dir = os.environ.get("TRAVEL_PLANNER_DATA_DIR")
    if configured_dir:
        return Path(configured_dir).expanduser().resolve()

    # The skill lives at <project>/.agents/skills/travel-planner/scripts.
    project_root = Path(__file__).resolve().parents[4]
    return project_root / ".codex-travel" / "travel-planner"
...
def ensure_db_files() -> None:
    ...
    if not TRIPS_FILE.exists():
        ...
        TRIPS_FILE.write_text(json.dumps(default_trips, indent=2), encoding="utf-8")
