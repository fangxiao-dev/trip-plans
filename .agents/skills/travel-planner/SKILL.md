...
## Windows + Codex (project-local)

In this workspace, keep all persistent skill data inside `<project>/.codex-travel/travel-planner/`; never use a user-home data directory. The database script uses that directory automatically, and the PowerShell wrappers work from Windows and Codex without requiring `python3` on `PATH`.
...
Set `TRAVEL_PLANNER_DATA_DIR` only when an explicit project-local override is needed. See [WINDOWS_CODEX.md](WINDOWS_CODEX.md) for the storage contract and commands.
...
**Data Storage:**
- Preferences: `<project>/.codex-travel/travel-planner/preferences.json`
- Trips: `<project>/.codex-travel/travel-planner/trips.json`
...
.\scripts\travel-db.ps1 stats

# Generate plan
.\scripts\generate-plan.ps1 --trip-id <id> --output plan.json

# Export backup
.\scripts\travel-db.ps1 export | Set-Content -Encoding utf8 backup.json
```
