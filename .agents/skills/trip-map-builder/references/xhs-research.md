...
所有新增状态都位于 `<project>/.codex-travel/trip-map-builder/`：Chrome profile、OpenCLI 本地安装、Browser Bridge 解压目录和 `MEMORY.md` 均不写入用户目录。配置文件为 `config/windows-codex.json`。
...
安装为项目本地依赖，不修改全局 npm 或用户 PATH。验证：
...
2. 解压到 `<project>/.codex-travel/trip-map-builder/opencli-extension`
...
- `opencli` 包装命令：`<skill>/scripts/opencli.ps1`
- OpenCLI 安装目录：`<project>/.codex-travel/trip-map-builder/tools/node_modules/@jackwener/opencli`
- CDP 实现：`<project>/.codex-travel/trip-map-builder/tools/node_modules/@jackwener/opencli/dist/src/browser/cdp.js`
- Browser Bridge 扩展：`<project>/.codex-travel/trip-map-builder/opencli-extension`
...
const projectRoot = process.env.TRIP_MAP_PROJECT_ROOT ?? process.cwd();
const cdpModule = path.join(projectRoot, '.codex-travel', 'trip-map-builder', 'tools', 'node_modules', '@jackwener', 'opencli', 'dist', 'src', 'browser', 'cdp.js');
const { CDPBridge } = await import(pathToFileURL(cdpModule).href);
