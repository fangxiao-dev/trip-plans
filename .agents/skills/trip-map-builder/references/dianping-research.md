...
.\scripts\opencli.ps1 dianping search "有乐町 晚餐" --city 东京 --limit 5 -f json
.\scripts\opencli.ps1 dianping search "新宿 居酒屋" --city 东京 --limit 5 -f json
```

命令格式：

```powershell
.\scripts\opencli.ps1 dianping search "<keyword>" --city <name-or-id> --limit <n> -f json
```
...
```powershell
.\scripts\opencli.ps1 dianping shop <shop_id> -f json
.\scripts\opencli.ps1 dianping detail <shop_id> -f json
```

也可以传完整店铺 URL：

```powershell
.\scripts\opencli.ps1 dianping shop "https://www.dianping.com/shop/<shop_id>"
```
