...
技能会优先读取 `<项目根目录>/.codex-travel/trip-map-builder/MEMORY.md`，复用长期有效的旅行偏好：节奏、餐饮、预算、支付、导航方式、历史行程输出和未解决事项。它不保存原始截图、证件、订单号或完整聊天记录。
...
├── config/
│   └── windows-codex.json    # 项目内数据与 CDP 配置
├── scripts/
│   ├── setup-windows-codex.ps1
...
运行时目录统一位于项目根目录 `.codex-travel/trip-map-builder/`，包括 Chrome profile、OpenCLI 安装、Browser Bridge 扩展和共享记忆。详见 [`WINDOWS_CODEX.md`](WINDOWS_CODEX.md)。
